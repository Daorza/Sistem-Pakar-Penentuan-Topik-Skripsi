...
Public Class FormPertanyaanAkhir
    ' Timer tracking
    ...
    Private selectedTopicId As Integer = 0

    ''' <summary>
    ''' Form Load - Load Level 2 questions based on top topic
    ''' </summary>
    Private Sub FormPertanyaanAkhir_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Try
            Dim userId As Integer = GlobalUser.UserId

            ' Get top ranked topic from Level 1
            Dim topicRankings = ResultRepository.GetRankingsByUser(userId, "TOPIK")

            If topicRankings.Count = 0 Then
                MessageBox.Show("Tidak ada hasil Level 1 ditemukan. Silakan ulangi tes.",
            ...
            ' Get top topic ID
            selectedTopicId = topicRankings.First().RefId

            ' Load Level 2 questions for this topic
            questions = QuestionRepository.GetLevel2QuestionsByTopic(selectedTopicId)

            If questions.Count = 0 Then
                MessageBox.Show($"Tidak ada pertanyaan Level 2 untuk topik terpilih.",
                              "Error", MessageBoxButtons.OK, MessageBoxIcon.Error)
                Me.Close()
                Return
            End If
        ...
        Catch ex As Exception
            MessageBox.Show("Gagal memuat pertanyaan Level 2: " & ex.Message, "Error",
    ...
    Private Sub buttonSelanjutnya_Click(sender As Object, e As EventArgs) Handles buttonSelanjutnya.Click
        ...
        If currentIndex = questions.Count - 1 Then
            ...
            If MessageBox.Show("Selesai menjawab semua pertanyaan Level 2. Lanjutkan ke hasil akhir?",
    ...
    ''' <summary>
    ''' Save all Level 2 answers to database
    ''' </summary>
    Private Sub SimpanJawabanKeDatabase()
        ...
        Catch ex As Exception
            MessageBox.Show("Gagal menyimpan jawaban Level 2: " & ex.Message, "Error",
                          MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub

    ''' <summary>
    ''' Process Level 2 results - Calculate CF for subtopics and save rankings
    ''' </summary>
    Private Sub ProcessLevel2Results()
        questionTimer.Stop()
        ...
        Try
            ...
            ' Calculate CF for subtopics of selected topic
            Dim subtopicCF As Dictionary(Of Integer, Double) = engine.CalculateSubtopicCF(userId, selectedTopicId)

            ' Save subtopic rankings
            Dim subtopicRankings = subtopicCF.OrderByDescending(Function(x) x.Value).ToList()

            For i As Integer = 0 To subtopicRankings.Count - 1
                Dim subtopicId As Integer = subtopicRankings(i).Key
                Dim cfValue As Double = subtopicRankings(i).Value
                Dim persentase As Integer = CertaintyFactorHelper.CFToPercentage(cfValue)
                Dim ranking As Integer = i + 1

                ResultRepository.InsertRanking(userId, "SUBTOPIK", subtopicId, cfValue, persentase, ranking)
            Next

            ' Navigate to Hasil Akhir
            Dim frmHasilAkhir As New FormHasilAkhir()
            frmHasilAkhir.Show()
            Me.Close()

        Catch ex As Exception
            MessageBox.Show("Gagal memproses hasil Level 2: " & ex.Message, "Error",
                          MessageBoxButtons.OK, MessageBoxIcon.Error)
        End Try
    End Sub
End Class
